...
## 分支说明
 - dev: 基于上游社区3.7.12分支的开发主分支，请勿从此分支拉取源码用于商用版本，不保证质量。
 - br_3.7.12-ohos-X.X.X: 基于上游社区3.7.12分支适配ohos后的发布分支。如需下载源码，请从此分支下载上述正式版本对应的tag点源码。
 - 3.22.0-ohos：基于上游社区3.22.0分支的开发主分支，如需下载源码，请下载3.22.0正式版本对应的tag点源码。
 - 3.22.1-ohos-X.X.X: 基于上游社区3.22.0分支适配ohos后的发布分支。
 - oh-3.27.4-dev: 这个分支是基于上游社区3.27.4分支适配ohos的分支，目前正在适配中，未商用发布，不保证质量。预计在2025年Q3发布。
...
7. 运行debug版本的flutter应用用到OpenHarmony设备后报错（release和profile版本正常）。  
   报错信息：`Error while initializing the Dart VM: Wrong full snapshot version, expected '8af474944053df1f0a3be6e6165fa7cf' found 'adb4292f3ec25074ca70abcd2d5c7251'`。  
   ...
   补充信息：若运行android或ios也出现类似错误，可以尝试还原环境变量   `FLUTTER_STORAGE_BASE_URL` ，清除缓存后重新运行。 
...
12. 执行 `flutter build hap` 时遇到路径校验报错。
    ...
    解决方案：打开deveco安装路径D:\DevEco Studio\tools\hvigor\hvigor-ohos-plugin\res\schemas下的 ohos-project-build-profile-schema.json文件。在该文件中找到包含：`"pattern": "^(\\./|\\.\\./)[\\s\\S]+$"`的行，并删除此行。
