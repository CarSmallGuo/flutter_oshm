...
const double _kMinScaleFactor = 0.85;

/// Reset DPI scale value used to clear overflow state
const double _kResetDpiScale = -1.0;
...
FlexOverflowStrategy createDefaultOverflowStrategy() {
  if (defaultTargetPlatform == TargetPlatform.ohos && kReleaseMode) {
...
class OhosFlexOverflowStrategy implements FlexOverflowStrategy {
  ...
  Future<void> _reportFlexOverflow(double dpiScale) async {
    print('quanhonghui Reporting flex overflow with scale: $dpiScale');
    try {
  ...
  bool _isRootVerticalFlex(RenderFlex renderFlex) {
    ...
    RenderObject? ancestor = renderFlex.parent as RenderObject?;
    while (ancestor != null) {
