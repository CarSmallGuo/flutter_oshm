...
function setFlutterHarInOverrides(
  ...
  let flutterHarPath = localEngineSrcPath && localEngine
    ? path.join(localEngineSrcPath, "out", localEngine, `flutter_embedding_${buildMode}.har`)
  ...
  targetPlatforms?.forEach(platform => {
    let arch = PLATFORM_ARCH_MAP[platform].replace('-', '_')
    let platformHarPath = localEngineSrcPath && localEngine
      ? path.join(localEngineSrcPath, "out", localEngine, `${arch}_${buildMode}.har`)
      : path.join(sdkPath, 'bin', 'cache', 'artifacts', 'engine',
        `${platform}${buildModeSuffix}`,
        `${arch}_${buildMode}.har`)
    overrides[`flutter_native_${arch}`] = `file:${platformHarPath}`
  })
}
